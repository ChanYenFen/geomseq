...
//
// The last version of sort_curves.cpp that carried the windowed 2-opt path,
// kept verbatim as the reference implementation of an approach that was
// measured and then removed. It is here so the code can be read again without
// reconstructing it, not because it is expected to come back: the measurements
// that retired it are in benchmarks/results/ (sort_curves_crossover,
// sort_curves_density, sort_curves_passes) and the reasoning is in CLAUDE.md.
//
// Read that reasoning before reviving any of this. The short version: the
// windowed path tests only the K=500 spatially nearest candidate edges, and
// K's coverage falls as n grows, so tour quality degrades with scale by
// design -- 6.4% worse at n=12,000 and 13.3% at 50,000 at constant density.
// Widening K restores quality only by giving back the speed that was the whole
// point.
//
// (This file replaced archive/sort_curves_v1_windowed2opt_backup.cpp, which
// despite its name contained no windowed code at all -- it was the backup taken
// *before* windowing, and its exhaustive loop still ships verbatim in
// sort_curves.cpp, so nothing was lost by retiring it.)
//
// sort_curves.cpp
